%% Reworked template from Ivanonv Sergey to use min-hash algorithm
...
if ~exist('hm', 'var')
    ...
    load('/mnt/Data/arutar/HASH/hm_500k_30.mat');
    load('/mnt/Data/arutar/HASH/data.mat');
    load('/mnt/Data/arutar/HASH/min_hashes_30.mat');
...
%     [ranks,ix] = sort(max(ranks), 'descend');
    ...
    try
        ranks = query1M(hm, queries{1}, C_255_u, C_255_d, kdtree_u, kdtree_d, hash_funs, N, subs, min_hashes);
    catch err
        fprintf('Couldn''t process img "%s"\n', queries{1});
    end
